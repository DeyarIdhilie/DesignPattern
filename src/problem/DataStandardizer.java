...
public class DataStandardizer {
	...
	protected void createAndShowGUI() {
		...
		topPanel = new JPanel();
                //C:\Users\lenovo\Documents\NetBeansProjects\AdvanceClassActivity\src\input_output\io.gogl
	...
	public void parse(String path) {
		...
		try (BufferedReader reader = Files.newBufferedReader(Paths.get(path), charset)) {
		    ...
		    {
                        Context context = new Context(new Google());
		        this.data = context.excuteStrategy(reader);
		    }
		    if (company.compareTo("microsoft")==0)
		    {
                        Context context = new Context(new Microsoft());
		        this.data = context.excuteStrategy(reader);
		    	 
		    }
                    
		  if (company.compareTo("amazon")==0)
		    {
                        Context context = new Context(new Aws());
		        this.data = context.excuteStrategy(reader);
